...
var requeueResult = runtime.Result{RequeueAfter: 10 * time.Second, Requeue: true}
...
func (reconciler *ClusterReconciler) reconcile() (runtime.Result, error) {
	var err error
	...
	return reconciler.reconcileJob()
}
...
func (reconciler *ClusterReconciler) reconcileJob() (runtime.Result, error) {
	...
	var newSavepointStatus *apiv1.SavepointStatus
	var newControlStatus *apiv1.FlinkClusterControlStatus
	...
	if desiredJob == nil && activeFlinkJob {
		...
		if savepointStatus != nil && savepointStatus.State != apiv1.SavepointStateSucceeded {
			return requeueResult, nil
		}
...
func (reconciler *ClusterReconciler) cancelRunningJobs(
	...
	if flinkJob != nil && flinkJob.ID != "" &&
		getFlinkJobDeploymentState(flinkJob.Status) == apiv1.JobStateRunning {
...
func (reconciler *ClusterReconciler) cancelFlinkJobAsync(jobID string, takeSavepoint bool) (*apiv1.SavepointStatus, error) {
	var log = reconciler.log
	...
	case apiv1.SavepointStateNotTriggered:
		if takeSavepoint && canTakeSavepoint(*reconciler.observed.cluster) {
			savepointStatus, err = reconciler.takeSavepointAsync(jobID, apiv1.SavepointTriggerReasonJobCancel)
			if err != nil {
...
func (reconciler *ClusterReconciler) shouldTakeSavepoint() (bool, string) {
	...
	if savepointStatus != nil && savepointStatus.State == apiv1.SavepointStateNotTriggered {
	...
	if jobSpec.SavepointGeneration > jobStatus.SavepointGeneration &&
		(savepointStatus != nil && savepointStatus.State != apiv1.SavepointStateFailed && savepointStatus.State != apiv1.SavepointStateTriggerFailed) {
		log.Info(
			"Savepoint is requested",
			"statusGen", jobStatus.SavepointGeneration,
			"specGen", jobSpec.SavepointGeneration)
		return true, apiv1.SavepointTriggerReasonUserRequested
	}
	...
	if len(jobStatus.LastSavepointTime) == 0 {
		return true, apiv1.SavepointTriggerReasonScheduledInitial
	}
	...
	return time.Now().After(nextTime), apiv1.SavepointTriggerReasonScheduled
}
...
func (reconciler *ClusterReconciler) takeSavepointAsync(jobID string, triggerReason string) (*apiv1.SavepointStatus, error) {
	var log = reconciler.log
	var cluster = reconciler.observed.cluster
	var apiBaseURL = getFlinkAPIBaseURL(reconciler.observed.cluster)
	...
	if requestedSavepoint != nil && requestedSavepoint.State == apiv1.SavepointStateNotTriggered {
...
func (reconciler *ClusterReconciler) updateSavepointStatus(
	...
	if controlStatus != nil && controlStatus.Name == apiv1.ControlNameSavepoint &&
...
func getFailedCancelStatus(cancelErr error) *apiv1.FlinkClusterControlStatus {
	var state string
	var message string
	var now string
	setTimestamp(&now)
	state = apiv1.ControlStateProgressing
	// limit message size to 1KiB
	if message = cancelErr.Error(); len(message) > 1024 {
		message = message[:1024] + "..."
	}
	return &apiv1.FlinkClusterControlStatus{
		Name:       apiv1.ControlNameJobCancel,
		State:      state,
		UpdateTime: now,
		Message:    message,
	}
...
func (reconciler *ClusterReconciler) updateStatus(ss **apiv1.SavepointStatus, cs **apiv1.FlinkClusterControlStatus) {
	var log = reconciler.log
	...
	retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		var clusterUpdated apiv1.FlinkCluster
