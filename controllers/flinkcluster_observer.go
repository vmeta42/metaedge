...
func (observer *ClusterStateObserver) observe(
	...
	// Cluster state.
	var observedCluster = new(apiv1.FlinkCluster)
...
func (observer *ClusterStateObserver) observeJob(
	...
	var jobInPendingState = recordedJobStatus != nil && recordedJobStatus.State == apiv1.JobStatePending
...
func (observer *ClusterStateObserver) observeFlinkJobStatus(
	...
	var jmReady = observed.jmStatefulSet != nil && getStatefulSetState(observed.jmStatefulSet) == apiv1.ComponentStateReady
	if !jmReady {
		log.Info(
	...
	for _, job := range flinkJobList.Jobs {
		if flinkJobID == job.ID {
			...
			*flinkJob = job
		} else if getFlinkJobDeploymentState(job.Status) == apiv1.JobStateRunning {
...
func (observer *ClusterStateObserver) observeSavepoint(observed *ObservedClusterState) error {
	...
	if savepointStatus != nil && savepointStatus.State == apiv1.SavepointStateInProgress {
		var flinkAPIBaseURL = getFlinkAPIBaseURL(observed.cluster)
...
func (observer *ClusterStateObserver) observeCluster(
	cluster *apiv1.FlinkCluster) error {
	return observer.k8sClient.Get(observer.context, observer.request.NamespacedName, cluster)
}
...
func (observer *ClusterStateObserver) observeRevisions(
	...
	selector := labels.SelectorFromSet(map[string]string{history.ControllerRevisionManagedByLabel: cluster.GetName()})
