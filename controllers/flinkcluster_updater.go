...
func (updater *ClusterStatusUpdater) updateStatusIfChanged() (
	...
	var oldStatus = apiv1.FlinkClusterStatus{}
...
func (updater *ClusterStatusUpdater) createStatusChangeEvents(
	oldStatus apiv1.FlinkClusterStatus,
	newStatus apiv1.FlinkClusterStatus) {
...
func (updater *ClusterStatusUpdater) deriveClusterStatus(
	recorded *apiv1.FlinkClusterStatus,
	observed *ObservedClusterState) apiv1.FlinkClusterStatus {
	var status = apiv1.FlinkClusterStatus{}
	...
	var isJobUpdating = recorded.Components.Job != nil && recorded.Components.Job.State == apiv1.JobStateUpdating
	...
	if !isComponentUpdated(observedJmStatefulSet, *observed.cluster) && isJobUpdating {
		...
		status.Components.JobManagerStatefulSet.State = apiv1.ComponentStateUpdating
		...
		if status.Components.JobManagerStatefulSet.State == apiv1.ComponentStateReady {
			runningComponents++
		}
	...
	if !isComponentUpdated(observedJmService, *observed.cluster) && isJobUpdating {
		...
		status.Components.JobManagerService.State = apiv1.ComponentStateUpdating
		...
		case corev1.ServiceTypeClusterIP:
			if observedJmService.Spec.ClusterIP != "" {
				state = apiv1.ComponentStateReady
				runningComponents++
			} else {
				state = apiv1.ComponentStateNotReady
			}
	...
	if !isComponentUpdated(observedJmIngress, *observed.cluster) && isJobUpdating {
		status.Components.JobManagerIngress = &apiv1.JobManagerIngressStatus{}
		recorded.Components.JobManagerIngress.DeepCopyInto(status.Components.JobManagerIngress)
		status.Components.JobManagerIngress.State = apiv1.ComponentStateUpdating
	...
	if !isComponentUpdated(observedTmStatefulSet, *observed.cluster) && isJobUpdating {
		...
		status.Components.TaskManagerStatefulSet.State = apiv1.ComponentStateUpdating
	...
	if jobStatus != nil &&
		(jobStatus.State == apiv1.JobStateSucceeded ||
			jobStatus.State == apiv1.JobStateFailed ||
			jobStatus.State == apiv1.JobStateCancelled ||
			jobStatus.State == apiv1.JobStateSuspended) {
		jobStopped = true
	}

	// Derive the new cluster state.
	switch recorded.State {
	case "", apiv1.ClusterStateCreating:
		if runningComponents < totalComponents {
			status.State = apiv1.ClusterStateCreating
		} else {
			status.State = apiv1.ClusterStateRunning
		}
	case apiv1.ClusterStateUpdating:
		if isClusterUpdating {
			status.State = apiv1.ClusterStateUpdating
		} else if runningComponents < totalComponents {
			if isUpdateTriggered(*recorded) {
				status.State = apiv1.ClusterStateUpdating
			} else {
				status.State = apiv1.ClusterStateReconciling
			}
		} else {
			status.State = apiv1.ClusterStateRunning
		}
	case apiv1.ClusterStateRunning,
		apiv1.ClusterStateReconciling:
		if isClusterUpdating {
			status.State = apiv1.ClusterStateUpdating
		} else if jobStopped {
			var policy = observed.cluster.Spec.Job.CleanupPolicy
			if jobStatus.State == apiv1.JobStateSucceeded &&
				policy.AfterJobSucceeds != apiv1.CleanupActionKeepCluster {
				status.State = apiv1.ClusterStateStopping
			} else if jobStatus.State == apiv1.JobStateFailed &&
				policy.AfterJobFails != apiv1.CleanupActionKeepCluster {
				status.State = apiv1.ClusterStateStopping
			} else if jobStatus.State == apiv1.JobStateCancelled &&
				policy.AfterJobCancelled != apiv1.CleanupActionKeepCluster {
				status.State = apiv1.ClusterStateStopping
			} else {
				status.State = apiv1.ClusterStateRunning
			}
	...
	case apiv1.ClusterStateStopping,
		apiv1.ClusterStatePartiallyStopped:
		if isClusterUpdating {
			status.State = apiv1.ClusterStateUpdating
		} else if runningComponents == 0 {
			status.State = apiv1.ClusterStateStopped
		} else if runningComponents < totalComponents {
			status.State = apiv1.ClusterStatePartiallyStopped
		} else {
			status.State = apiv1.ClusterStateStopping
		}
	case apiv1.ClusterStateStopped:
		if isUpdateTriggered(*recorded) {
			status.State = apiv1.ClusterStateUpdating
		} else {
			status.State = apiv1.ClusterStateStopped
	...
	if recorded.Savepoint != nil {
		...
		if recorded.Savepoint.State == apiv1.SavepointStateInProgress && observed.savepoint != nil {
			switch {
			case observed.savepoint.IsSuccessful():
				newSavepointStatus.State = apiv1.SavepointStateSucceeded
			case observed.savepoint.IsFailed():
				var msg string
				newSavepointStatus.State = apiv1.SavepointStateFailed
				...
				if newSavepointStatus.TriggerReason == apiv1.SavepointTriggerReasonUpdate {
		...
		if newSavepointStatus.State == apiv1.SavepointStateNotTriggered || newSavepointStatus.State == apiv1.SavepointStateInProgress {
			...
			// TODO: organize more making savepoint status
			if newSavepointStatus.State == apiv1.SavepointStateFailed &&
				newSavepointStatus.TriggerReason == apiv1.SavepointTriggerReasonUpdate {
				newSavepointStatus.Message =
					"Failed to take savepoint for update. " +
						"The update process is being postponed until a savepoint is available. " + newSavepointStatus.Message
			}
	...
	var userControl = observed.cluster.Annotations[apiv1.ControlAnnotation]

	// Update job control status in progress
	var controlStatus *apiv1.FlinkClusterControlStatus
	if recorded.Control != nil && userControl == recorded.Control.Name &&
		recorded.Control.State == apiv1.ControlStateProgressing {
		controlStatus = recorded.Control.DeepCopy()
		var savepointStatus = status.Savepoint
		switch recorded.Control.Name {
		case apiv1.ControlNameJobCancel:
			if status.Components.Job.State == apiv1.JobStateCancelled {
				controlStatus.State = apiv1.ControlStateSucceeded
				...
				setTimestamp(&controlStatus.UpdateTime)
			} else if savepointStatus != nil && savepointStatus.State == apiv1.SavepointStateFailed {
		...
		case apiv1.ControlNameSavepoint:
			if savepointStatus != nil {
				if savepointStatus.State == apiv1.SavepointStateSucceeded {
					controlStatus.State = apiv1.ControlStateSucceeded
					setTimestamp(&controlStatus.UpdateTime)
				} else if savepointStatus.State == apiv1.SavepointStateFailed || savepointStatus.State == apiv1.SavepointStateTriggerFailed {
					controlStatus.State = apiv1.ControlStateFailed
					setTimestamp(&controlStatus.UpdateTime)
				}
			}
		...
		if userControl != apiv1.ControlNameJobCancel && userControl != apiv1.ControlNameSavepoint {
			if userControl != "" {
				updater.log.Info(fmt.Sprintf(apiv1.InvalidControlAnnMsg, apiv1.ControlAnnotation, userControl))
			}
		} else if recorded.Control != nil && recorded.Control.State == apiv1.ControlStateProgressing {
			updater.log.Info(fmt.Sprintf(apiv1.ControlChangeWarnMsg, apiv1.ControlAnnotation), "current control", recorded.Control.Name, "new control", userControl)
		} else {
			switch userControl {
			case apiv1.ControlNameSavepoint:
				if observed.cluster.Spec.Job.SavepointsDir == nil || *observed.cluster.Spec.Job.SavepointsDir == "" {
					updater.log.Info(fmt.Sprintf(apiv1.InvalidSavepointDirMsg, apiv1.ControlAnnotation))
					break
				} else if isJobStopped(observed.cluster.Status.Components.Job) {
					updater.log.Info(fmt.Sprintf(apiv1.InvalidJobStateForSavepointMsg, apiv1.ControlAnnotation))
					break
				}
				// Clear status for new savepoint
				status.Savepoint = getRequestedSavepointStatus(apiv1.SavepointTriggerReasonUserRequested)
			...
			case apiv1.ControlNameJobCancel:
				if isJobTerminated(observed.cluster.Spec.Job.RestartPolicy, recorded.Components.Job) {
					updater.log.Info(fmt.Sprintf(apiv1.InvalidJobStateForJobCancelMsg, apiv1.ControlAnnotation))
					break
				}
				// Savepoint for job-cancel
				var observedSavepoint = observed.cluster.Status.Savepoint
				if observedSavepoint == nil ||
					(observedSavepoint.State != apiv1.SavepointStateInProgress && observedSavepoint.State != apiv1.SavepointStateNotTriggered) {
					updater.log.Info("There is no savepoint in progress. Trigger savepoint in reconciler.")
					status.Savepoint = getRequestedSavepointStatus(apiv1.SavepointTriggerReasonJobCancel)
				} else {
	...
	// Handle update.
	var savepointForJobUpdate *apiv1.SavepointStatus
	switch updateState {
	case UpdateStatePreparing:
		...
		if !isSavepointUpToDate(observed.observeTime, *jobStatus) &&
			...
			(recorded.Savepoint == nil || recorded.Savepoint.State != apiv1.SavepointStateNotTriggered) {
			...
			if recorded.Savepoint != nil &&
				...
				status.Savepoint = getRequestedSavepointStatus(apiv1.SavepointTriggerReasonUpdate)
				updater.log.Info("Savepoint will be triggered for update")
			}
		} else if recorded.Savepoint != nil && recorded.Savepoint.State == apiv1.SavepointStateInProgress {
...
func (updater *ClusterStatusUpdater) getJobStatus() *apiv1.JobStatus {
	...
	var recordedJobStatus = updater.observed.cluster.Status.Components.Job
	var newJobStatus *apiv1.JobStatus
	...
	case isUpdateTriggered(observedCluster.Status) &&
		(isJobStopped(recordedJobStatus) || observedCluster.Status.State == apiv1.ClusterStateStopped):
		jobState = apiv1.JobStateUpdating
	...
	case isFlinkAPIReady(observed):
		...
		case apiv1.JobStateRunning:
			jobState = apiv1.JobStateLost
		case apiv1.JobStatePending:
			...
			if jobSubmitSucceeded || jobSubmitInProgress {
				jobState = apiv1.JobStatePending
				break
			}
			jobState = apiv1.JobStateFailed
		default:
			jobState = recordedJobStatus.State
		}
	// When Flink API unavailable
	default:
		if recordedJobStatus.State == apiv1.JobStatePending {
	...
	// Flink Job ID
	if jobState == apiv1.JobStateUpdating {
		newJobStatus.ID = ""
...
func (updater *ClusterStatusUpdater) isStatusChanged(
	currentStatus apiv1.FlinkClusterStatus,
	newStatus apiv1.FlinkClusterStatus) bool {
	var changed = false
...
func (updater *ClusterStatusUpdater) updateClusterStatus(
	status apiv1.FlinkClusterStatus) error {
	var cluster = apiv1.FlinkCluster{}
...
func (updater *ClusterStatusUpdater) clearControlAnnotation(newControlStatus *apiv1.FlinkClusterControlStatus) error {
	var userControl = updater.observed.cluster.Annotations[apiv1.ControlAnnotation]
	if userControl == "" {
		return nil
	}
...
func getStatefulSetState(statefulSet *appsv1.StatefulSet) string {
	if statefulSet.Status.ReadyReplicas >= *statefulSet.Spec.Replicas {
		return apiv1.ComponentStateReady
	}
	return apiv1.ComponentStateNotReady
}
