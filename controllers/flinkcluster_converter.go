...
func getDesiredJobManagerStatefulSet(
	flinkCluster *apiv1.FlinkCluster) *appsv1.StatefulSet {
	...
	if jobManagerStatefulSet.Spec.VolumeClaimTemplates != nil {
		for i, template := range jobManagerStatefulSet.Spec.VolumeClaimTemplates {
			if template.Name == "" {
				jobManagerStatefulSet.Spec.VolumeClaimTemplates[i].Name = "data"
			}
		}
	}
...
func getDesiredJobManagerService(
	flinkCluster *apiv1.FlinkCluster) *corev1.Service {
	...
	case apiv1.AccessScopeExternal:
		jobManagerService.Spec.Type = corev1.ServiceTypeLoadBalancer
	case apiv1.AccessScopeNodePort:
		jobManagerService.Spec.Type = corev1.ServiceTypeNodePort
	case apiv1.AccessScopeHeadless:
...
func getDesiredJobManagerIngress(
	flinkCluster *apiv1.FlinkCluster) *extensionsv1beta1.Ingress {
...
func getDesiredTaskManagerStatefulSet(
	...
	if taskManagerStatefulSet.Spec.VolumeClaimTemplates != nil {
		for i, template := range taskManagerStatefulSet.Spec.VolumeClaimTemplates {
			if template.Name == "" {
				taskManagerStatefulSet.Spec.VolumeClaimTemplates[i].Name = "data"
			}
		}
	}
...
func getDesiredConfigMap(
	flinkCluster *apiv1.FlinkCluster) *corev1.ConfigMap {
...
func convertFromSavepoint(jobSpec *apiv1.JobSpec, jobStatus *apiv1.JobStatus) *string {
	switch {
	...
	case jobStatus.State == apiv1.JobStateUpdating && jobSpec.FromSavepoint != nil && *jobSpec.FromSavepoint != "":
		return jobSpec.FromSavepoint
	// Latest savepoint
...
func convertJobManagerInitContainers(jobManagerSpec *apiv1.JobManagerSpec) []corev1.Container {
	return ensureVolumeMountsInitContainer(jobManagerSpec.InitContainers, jobManagerSpec.VolumeMounts)
}

func convertTaskManagerInitContainers(taskSpec *apiv1.TaskManagerSpec) []corev1.Container {
	return ensureVolumeMountsInitContainer(taskSpec.InitContainers, taskSpec.VolumeMounts)
}

func convertJobInitContainers(jobSpec *apiv1.JobSpec) []corev1.Container {
...
func ToOwnerReference(
	flinkCluster *apiv1.FlinkCluster) metav1.OwnerReference {
...
func shouldCleanup(
	cluster *apiv1.FlinkCluster, component string) bool {
	...
	var action apiv1.CleanupAction
	switch jobStatus.State {
	case apiv1.JobStateSucceeded:
		action = cluster.Spec.Job.CleanupPolicy.AfterJobSucceeds
	case apiv1.JobStateFailed:
		action = cluster.Spec.Job.CleanupPolicy.AfterJobFails
	case apiv1.JobStateCancelled:
	...
	switch action {
	case apiv1.CleanupActionDeleteCluster:
		return true
	case apiv1.CleanupActionDeleteTaskManager:
...
func calFlinkHeapSize(cluster *apiv1.FlinkCluster) map[string]string {
...
func convertHadoopConfig(hadoopConfig *apiv1.HadoopConfig) (
...
func convertGCPConfig(gcpConfig *apiv1.GCPConfig) (*corev1.Volume, *corev1.VolumeMount, *corev1.EnvVar) {
	if gcpConfig == nil {
		return nil, nil, nil
	}
...
func getClusterLabels(cluster apiv1.FlinkCluster) map[string]string {
	return map[string]string{
		"cluster":                       cluster.Name,
		"daas.tenxcloud.com/flink-name": cluster.Name,
		"app":                           "flink",
	}
}
...
func getComponentLabels(cluster apiv1.FlinkCluster, component string) map[string]string {
	return mergeLabels(getClusterLabels(cluster), map[string]string{
		"component": component,
	})
}

func getRevisionHashLabels(status apiv1.FlinkClusterStatus) map[string]string {
	return map[string]string{
...
func getLogConf(spec apiv1.FlinkClusterSpec) map[string]string {
